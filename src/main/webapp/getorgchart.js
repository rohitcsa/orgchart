...
getOrgChart = function(b, a) {
	...
	this._aH = null;
	this._an = [];
	this._zd = new getOrgChart.node(-1, null, null, null, 2, 2);
	this._zh = {
		found : [],
		showIndex : 0,
		oldValue : "",
...
getOrgChart.prototype._as = function() {
	...
	var a = getOrgChart.INNER_HTML.replace("[theme]", this.config.theme)
			.replace("[color]", this.config.color).replace(/\[height]/g,
					this._aO).replace(/\[toolbar-height]/g,
...
getOrgChart.prototype.resize = function() {
	this._f();
	this._X._t.style.height = this._aO + "px";
	this._X._u.style.height = this._aO + "px";
	this._X._7.style.height = this._aO + "px"
};
getOrgChart.prototype._f = function() {
	this._zB = get._c().msie ? this.element.clientWidth : window
	...
	this._zG = get._c().msie ? this.element.clientHeight : window
			.getComputedStyle(this.element, null).height;
	this._zG = parseInt(this._zG);
	if (this._zG < 3) {
		this._zG = parseInt((this._zB * 9) / 16);
		this.element.style.height = this._zG + "px"
	}
	this._aL = this._zB;
	this._aO = this._zG - this.theme.toolbarHeight
};
getOrgChart.prototype.changeColorScheme = function(a) {
	if (this.config.color == a) {
		return
	}
	this._X._zT.className = this._X._zT.className.replace(this.config.color, a);
	this.config.color = a
};
getOrgChart.prototype._a4 = function() {
	this._ai = [];
	this._ak = [];
	this._zq = [];
	getOrgChart._F(this, this._zd, 0);
	getOrgChart._zm(this, this._zd, 0, 0, 0);
	getOrgChart._zX(this)
};
...
getOrgChart.prototype._A = function() {
	...
	e.push(getOrgChart.OPEN_SVG.replace("[defs]",
			this.config.embededDefinitions).replace("[viewBox]", g.toString()));
...
getOrgChart.prototype._r = function(a, i, c, b, g, h) {
	...
	var f = null;
	if (i) {
		f = i * ((this._aL / 100) / (b.w + g))
	}
	var e = null;
	if (c) {
		e = c * ((this._aO / 100) / (b.h + h))
	}
	if (f != null && e != null) {
		d = f > e ? e : f
	} else {
...
getOrgChart.prototype._6 = function() {
	...
	if (this.config.boxSizeInPercentage != null) {
		...
		this.scale = this._r(this.config.scale,
				this.config.boxSizeInPercentage.boxSize.width,
				this.config.boxSizeInPercentage.boxSize.height, c, l, m);
		this.minScale = this._r(0,
				this.config.boxSizeInPercentage.minBoxSize.width,
				this.config.boxSizeInPercentage.minBoxSize.height, c, l, m);
		this.maxScale = this._r(10000000,
				this.config.boxSizeInPercentage.maxBoxSize.width,
				this.config.boxSizeInPercentage.maxBoxSize.height, c, l, m)
	}
	switch (this.config.orientation) {
	case getOrgChart.RO_TOP:
	case getOrgChart.RO_TOP_PARENT_LEFT:
		...
		if (this.scale === "auto") {
			...
			o = (this._aL) / this.scale;
			d = (this._aO) / this.scale;
			if (this.config.orientation == getOrgChart.RO_TOP) {
				a = c.x - o / 2 + c.w / 2
			}
		}
		this.initialViewBoxMatrix = [ -p + a, q + b,
				o + this.config.siblingSeparation, d ];
		break;
	case getOrgChart.RO_BOTTOM:
	case getOrgChart.RO_BOTTOM_PARENT_LEFT:
		o = Math.abs(h) + Math.abs(f) + this.theme.size[0];
		...
		this.initialViewBoxMatrix = [ -p + a, -q - d - b,
				o + this.config.siblingSeparation, d ];
		break;
	case getOrgChart.RO_RIGHT:
	case getOrgChart.RO_RIGHT_PARENT_TOP:
		...
		this.initialViewBoxMatrix = [ -o - q - a, -p + b, o,
				d + this.config.siblingSeparation ];
		break;
	case getOrgChart.RO_LEFT:
	case getOrgChart.RO_LEFT_PARENT_TOP:
		o = Math.abs(h) + Math.abs(f) + this.theme.size[0];
		...
		this.initialViewBoxMatrix = [ q + a, -p + b, o,
				d + this.config.siblingSeparation ];
		break
	}
...
getOrgChart.prototype.draw = function(a) {
	...
	if (this.config.enableSearch) {
		...
		this._X._aX.style.display = "inherit";
		this._X._a9.style.display = "inherit"
	}
...
getOrgChart.prototype._z = function(a) {
	...
	for ( var d in this.nodes) {
		if (this.nodes[d]._zH == null || this.nodes[d]._zN == null) {
			continue
		}
		if (this.nodes[d]._zH == this.nodes[d].x
				&& this.nodes[d]._zN == this.nodes[d].y) {
			continue
		}
...
getOrgChart.prototype._aW = function(b, a) {
	this._zs(b, "mouseup", this._aW);
	this._zs(b, "mouseleave", this._aW);
...
getOrgChart.prototype.isCollapsed = function(a) {
	if ((a.parent == this._zd) || (a.parent == null)) {
		return false
	}
...
getOrgChart.prototype._e = function() {
	if (this.config.enableGridView) {
		this._q(this._X._8, "click", this._zQ);
		this._q(this._X._9, "click", this._zA)
	}
	if (this.config.enablePrint) {
	...
	if (this.config.enableMove) {
		if ("ontouchstart" in window) {
			...
			this._q(this._X._ze, "mousedown", this._aZ);
			this._q(this._X._au, "mousedown", this._aZ);
			this._q(this._X._Q, "mousedown", this._aZ);
			this._q(this._X._zV, "mousedown", this._aZ);
	...
	for (i = 0; i < this._X._aT.length; i++) {
		if ("ontouchstart" in window && "onorientationchange" in window) {
			this._q(this._X._aT[i], "touchstart", this._aY);
			this._q(this._X._aT[i], "touchmove", this._aB);
			this._q(this._X._aT[i], "touchend", this._aG)
		} else {
			this._q(this._X._aT[i], "mousedown", this._aY);
			this._q(this._X._aT[i], "click", this._aR);
			this._q(this._X._aT[i], "mouseover", this._aV)
		}
	}
	this._q(this._X._o, "click", this._zf);
	if (this.config.enableZoom) {
		this._q(this._X._zJ, "click", this._zM);
		this._q(this._X._zI, "click", this._zK);
		this._q(this._X._t, "DOMMouseScroll", this._zy);
		this._q(this._X._t, "mousewheel", this._zy)
	}
	if (this.config.enableSearch) {
		this._q(this._X._aX, "click", this._aE);
		this._q(this._X._a9, "click", this._a0);
		this._q(this._X._zn, "keyup", this._zu);
		this._q(this._X._zn, "paste", this._zj)
	}
	var b = "onorientationchange" in window, a = b ? "orientationchange"
			: "resize";
	this._q(window, a, this._zx);
	if ("ontouchstart" in window && "onorientationchange" in window) {
		this._q(this._X._u, "touchstart", this._zg, "detilsView");
		this._q(this._X._u, "touchmove", this._zt, "detilsView")
	}
};
...
getOrgChart.prototype._zs = function(a, b) {
...
getOrgChart.prototype._d = function() {
	if (this.config.removeNodeEvent) {
		this._zY("removeNodeEvent", this.config.removeNodeEvent)
	}
	if (this.config.updateNodeEvent) {
		this._zY("updateNodeEvent", this.config.updateNodeEvent)
	}
	if (this.config.createNodeEvent) {
		this._zY("createNodeEvent", this.config.createNodeEvent)
	}
	if (this.config.clickNodeEvent) {
		this._zY("clickNodeEvent", this.config.clickNodeEvent)
	}
	if (this.config.renderNodeEvent) {
		this._zY("renderNodeEvent", this.config.renderNodeEvent)
	}
	if (this.config.insertNodeEvent) {
		this._zY("insertNodeEvent", this.config.insertNodeEvent)
	}
	if (this.config.updatedEvent) {
		this._zY("updatedEvent", this.config.updatedEvent)
	}
};
...
getOrgChart._X.prototype._a7 = function() {
	this._zT = this.element.getElementsByTagName("div")[0];
	var a = this._zT.children;
	this._zE = a[0];
	this._t = a[1];
	this._u = a[2];
	this._7 = a[3]
};
getOrgChart._X.prototype._a6 = function() {
	this._v = this._t.getElementsByTagName("svg")[0];
	this._a3 = this._v.getElementsByTagName("g")[0];
	this._zD = this._zE.getElementsByTagName("div")[0];
	var d = this._zD.getElementsByTagName("div")[0];
	var a = this._zD.getElementsByTagName("div")[1];
	var b = this._zD.getElementsByTagName("div")[2];
	this._zn = d.getElementsByTagName("input")[0];
	...
	this._aD = this._a3.querySelectorAll("[data-btn-action]");
	this._aT = this._a3.querySelectorAll("[data-node-id]");
	...
	this._ze = this._zT.getElementsByClassName("get-right")[0];
	this._au = this._zT.getElementsByClassName("get-left")[0];
	this._Q = this._zT.getElementsByClassName("get-down")[0];
	this._zV = this._zT.getElementsByClassName("get-up")[0]
};
...
getOrgChart._X.prototype.getNodeById = function(a) {
	return this._a3.querySelector("[data-node-id='" + a + "']")
};
getOrgChart._X.prototype.removeLinks = function() {
	var a = this._a3.querySelectorAll("[data-link-id]");
	var b = a.length;
	while (b--) {
...
getOrgChart._X.prototype.getButtonByType = function(a) {
	return this._a3.querySelector("[data-btn-action='" + a + "']")
};
getOrgChart._X.prototype._zF = function(a) {
	var c;
	if (!a) {
		c = this._zS
	} else {
	...
	for (i = 0; i < c.length; i++) {
		...
		while (b > d) {
			c[i].textContent = c[i].textContent.substring(0,
					c[i].textContent.length - 4);
			c[i].textContent += "...";
...
getOrgChart.INNER_HTML = '<div class="get-[theme] get-[color] get-org-chart"><div class="get-oc-tb"><div><div style="height:[toolbar-height]px;"><input placeholder="Search" type="text" /><a title="previous" class="get-prev get-disabled" href="javascript:void(0)">&nbsp;</a><a title="next" class="get-next get-disabled" href="javascript:void(0)">&nbsp;</a><a class="get-minus" title="zoom out" href="javascript:void(0)" style="color:white">&nbsp;</a><a class="get-plus" title="zoom in" href="javascript:void(0)" style="color:white">&nbsp;</a><a href="javascript:void(0)" class="get-grid-view" title="grid view" style="color:white">&nbsp;</a><a href="javascript:void(0)" class="get-print" title="print"style="color:white">&nbsp;</a></div><div style="height:[toolbar-height]px;"><a title="previous page" class="get-prev-page" href="javascript:void(0)"style="color:white">&nbsp;</a></div><div style="height:[toolbar-height]px;"><a title="previous page" class="get-prev-page" href="javascript:void(0)" style="color:white">&nbsp;</a></div></div></div><div class="get-oc-c" style="height:[height]px;"></div><div class="get-oc-v" style="height:[height]px;"><div class="get-image-pane"></div><div class="get-data-pane"></div></div><div class="get-oc-g" style="height:[height]px;"></div><div class="get-left"><div class="get-left-icon" style="color:white"></div></div><div class="get-right" style="color:white"><div class="get-right-icon"style="color:white"></div></div><div class="get-up"><div class="get-up-icon" style="color:white"></div></div><div class="get-down"><div class="get-down-icon" style="color:white"></div></div></div>';
...
getOrgChart._F = function(h, g, d) {
	...
	if (g.collapsed == getOrgChart.NOT_DEFINED) {
		g.collapsed = (h.config.expandToLevel && h.config.expandToLevel <= d
				&& g._Y() ? getOrgChart.COLLAPSED : getOrgChart.EXPANDED)
	}
	if (g._Y() == 0 || d == h.config.maxDepth) {
		c = g._L();
		if (c != null) {
			g._a8 = c._a8 + h._P(c) + h.config.siblingSeparation
		} else {
			g._a8 = 0
		}
	} else {
...
getOrgChart._w = function(t, m, g) {
	...
	for (var d = t.config.maxDepth - g; a != null && b != null && c <= d;) {
		...
		var s = (b._a8 + h + t._P(b) + t.config.subtreeSeparation)
				- (a._a8 + i);
		if (s > 0) {
			var q = m;
			var n = 0;
...
getOrgChart._zm = function(h, d, b, i, k) {
	if (b <= h.config.maxDepth) {
		...
		if (d._Y() != 0) {
			getOrgChart._zm(h, d._I(), b + 1, i + d._ao, k + c
					+ h.config.levelSeparation)
		}
		var f = d._2();
		if (f != null) {
			getOrgChart._zm(h, f, b, i, k)
		}
	}
};
getOrgChart._zX = function(e) {
	e._zc = e._zd.x;
	e._zr = e._zd.y;
	if (e._aH) {
		var b = e.nodes[e._aH.id];
		var c = e._aH.old_x - b.x;
		var d = e._aH.old_y - b.y;
...
getOrgChart.node.prototype._L = function() {
	if (this.leftNeighbor != null
			&& this.leftNeighbor.getParent() == this.getParent()) {
...
getOrgChart.node.prototype._2 = function() {
	if (this.rightNeighbor != null
			&& this.rightNeighbor.getParent() == this.getParent()) {
...
getOrgChart.node.prototype._B = function(a) {
	node = this._I();
	node1 = this._K();
	return node._a8 + ((node1._a8 - node._a8) + a._P(node1)) / 2
};
...
getOrgChart.node.prototype._Z = function(a) {
	...
	var c = null;
	var e = a.customize[this.id] && a.customize[this.id].theme ? getOrgChart.themes[a.customize[this.id].theme]
			: getOrgChart.themes[a.theme];
	...
	for (var b = 0; b < this.getChildren().length; b++) {
		...
		if (this.displayChildren == null && c.displayChildren != null
				&& this.collapsed == getOrgChart.EXPANDED) {
			switch (a.orientation) {
			case getOrgChart.RO_TOP:
			case getOrgChart.RO_TOP_PARENT_LEFT:
			case getOrgChart.RO_BOTTOM:
			case getOrgChart.RO_BOTTOM_PARENT_LEFT:
				d.push('<path data-link-id="' + this.id
						+ '" class="link"   d="M' + p + "," + u + " " + q + ","
						+ v + " " + g + "," + v + " " + h + "," + n + " L" + j
						+ "," + o + '"/>');
				break;
			case getOrgChart.RO_RIGHT:
		...
		if (a.expandToLevel && this.displayChildren == null) {
			d
					.push(getOrgChart.buttons.expColl.replace(
							"[display]",
							this.collapsed == getOrgChart.EXPANDED ? "none"
									: "block").replace(/\[xa]/g,
							(p - e.expandCollapseBtnRadius)).replace(/\[ya]/g,
							(u - e.expandCollapseBtnRadius)).replace(
							/\[start]/g, ((e.expandCollapseBtnRadius * 2) / 6))
							.replace(/\[middle]/g, (e.expandCollapseBtnRadius))
							.replace(/\[end]/g,
									((e.expandCollapseBtnRadius * 2 / 6) * 5))
...
getOrgChart.node.prototype._W = function(c) {
	...
	var e = null;
	var g = c.customize[this.id] && c.customize[this.id].theme ? getOrgChart.themes[c.customize[this.id].theme]
			: getOrgChart.themes[c.theme];
	...
	for (var d = this.secondChildren.length - 1; d >= 0; d--) {
		...
		if (this.collapsed == getOrgChart.EXPANDED) {
			f.push('<path data-link-id="' + this.id
					+ '" class="link-second-parent"   d="M' + a + "," + b
					+ "  L" + j + "," + m + '"/>')
		}
	}
	return f.join("")
...
getOrgChart.node.prototype.draw = function(a) {
	...
	var m = a.customize[this.id] && a.customize[this.id].theme ? getOrgChart.themes[a.customize[this.id].theme]
			: getOrgChart.themes[a.theme];
	var f = a.customize[this.id] && a.customize[this.id].theme ? " get-"
			+ a.customize[this.id].theme : "";
	var e = a.customize[this.id] && a.customize[this.id].color ? " get-"
			+ a.customize[this.id].color : "";
	if (f && !e) {
		e = " get-" + a.color
	}
	if (this.color != null
			&& (this.displayChildren != null || this.displayParent != null)) {
		e += " get-" + this.color
	}
	if (e && !f) {
		f = " get-" + a.theme
	...
	h.push(getOrgChart.OPEN_NODE.replace("[data-node-id]", this.id).replace(
			"[x]", this._zH == null ? this.x : this._zH).replace("[y]",
			this._zN == null ? this.y : this._zN)
			.replace("[level]", this.level).replace("[nodeCssClass]", d));
	for (themeProperty in m) {
		...
		case "text":
			...
			for (k = 0; k < a.primaryFields.length; k++) {
				...
				h.push(m.text.replace("[index]", j).replace("[text]",
						this.data[c]).replace("[y]", g.y).replace("[x]", g.x)
						.replace("[rotate]", g.rotate).replace("[width]",
								g.width));
				j++
			}
			break;
		default:
			if (themeProperty != "size" && themeProperty != "toolbarHeight"
					&& themeProperty != "textPoints"
					&& themeProperty != "expandCollapseBtnRadius"
					&& themeProperty != "textPointsNoImage") {
...
get._z = function(a, c, b, h, j, d) {
	...
	function f() {
		for ( var s in b) {
			var t = getOrgChart.util._s([ "top", "left", "right", "bottom" ], s
					.toLowerCase()) ? "px" : "";
...
getOrgChart.util._3 = function(element) {
	...
	transform = transform.replace("matrix", "").replace("(", "").replace(")",
			"");
	transform = getOrgChart.util._zR(transform);
	transform = "[" + transform + "]";
	return eval(transform.replace(/\ /g, ", "))
};
getOrgChart.util._zR = function(a) {
...
getOrgChart.util._zZ = function(c) {
...
getOrgChart.util._a2 = function(b, a) {
	var c = b.getBoundingClientRect();
...
getOrgChart.util._zC = function(b, a) {
	var c = b.getBoundingClientRect();
	var d = a.touches[0].pageX - c.left;
	var f = a.touches[0].pageY - c.top;
...
getOrgChart.util._1 = function(a) {
	var b = [ "darkred", "pink", "darkorange", "orange", "lightgreen", "green",
			"lightteal", "teal", "lightblue", "blue", "darkpurple", "purple",
			"mediumdarkblue", "darkblue", "cordovan", "darkcordovan",
			"neutralgrey" ];
	var c = b.indexOf(a);
	b.splice(c, 1);
...
getOrgChart.prototype.showDetailsView = function(d) {
	var h = this.nodes[d];
	var f = (h.parent == this._zd);
	var b = function(p, n, q) {
		var l = f ? 'style="display:none;"' : "";
		var r = "<select " + l + 'class="get-oc-select-parent"><option value="'
	...
	for (label in h.data) {
		c += getOrgChart.DETAILS_VIEW_INPUT_HTML.replace(/\[label]/g, label)
				.replace("[value]", h.data[label]);
		g.push(label)
	}
	c += a(g, this._an);
	...
	if (e) {
		this._X._m.innerHTML = getOrgChart.DETAILS_VIEW_ID_IMAGE.replace(
				"[src]", e)
	} else {
		this._X._m.innerHTML = getOrgChart.DETAILS_VIEW_USER_LOGO
	}
	this._k();
	var j = this.config.customize[h.id] && this.config.customize[h.id].theme ? getOrgChart.themes[this.config.customize[h.id].theme].toolbarHeight
			: this.theme.toolbarHeight;
...
getOrgChart.prototype._zf = function(e, a) {
	if (this._S) {
...
getOrgChart.prototype.showMainView = function() {
	...
	this._X._7.style.left = "-9999px";
	this._X._7.innerHTML = "";
	if (this._X._zD.style.top != 0 && this._X._zD.style.top != ""
			&& this._X._zD.style.top != "0px") {
		get._z(this._X._zD, {
			top : -46
		}, {
			top : 0
...
getOrgChart.prototype.print = function() {
	var b = this, d = this._X.element, k = this._X._zE, g = [], h = d.parentNode, j = k.style.display, a = document.body, c = a.childNodes, e;
	if (b._ag) {
		return
	}
	b._ag = true;
...
getOrgChart.prototype._zM = function() {
	this.zoom(true, true)
};
getOrgChart.prototype._zK = function() {
	this.zoom(false, true)
};
getOrgChart.prototype._zg = function(b, a) {
	this._zb = this._X._u.scrollTop + a[0].touches[0].pageY
};
getOrgChart.prototype._zt = function(b, a) {
	this._X._u.scrollTop = this._zb - a[0].touches[0].pageY
};
getOrgChart.prototype._zy = function(d, b) {
...
getOrgChart.prototype._ap = function(g, b) {
	...
	if (this._aK.dragStart) {
		...
		var c = j[3] / this._aO;
		var a = k > c ? k : c;
		j[0] = -((e - this._aK.dragStart.x) * a)
				+ this._aK.dragStart.viewBoxLeft;
		j[1] = -((f - this._aK.dragStart.y) * a)
				+ this._aK.dragStart.viewBoxTop;
		j[0] = parseInt(j[0]);
		j[1] = parseInt(j[1]);
...
getOrgChart.prototype.zoom = function(c, a, b) {
	...
	if (this.scale < this.minScale) {
		...
		j[2] = this._aL / this.scale;
		j[3] = this._aO / this.scale;
		j[0] = f[0] - (j[2] - h) / (100 / b[0]);
		j[1] = f[1] - (j[3] - g) / (100 / b[1])
	}
...
getOrgChart.prototype._aE = function(c, b) {
	...
	var a = this;
	clearTimeout(this._zh.timer);
	this._zh.timer = setTimeout(function() {
		a._zh.currentIndex++;
		a._l();
		a._0()
	}, 100)
...
getOrgChart.prototype._zu = function(c, b) {
	var a = this;
	clearTimeout(this._zh.timer);
	this._zh.timer = setTimeout(function() {
		a._zh.found = a._C(a._X._zn.value);
		a._zh.currentIndex = 0;
		a._l();
		a._ah();
		a._0()
	}, 500)
...
getOrgChart.prototype._0 = function() {
	if (this._zh.found.length) {
		this.highlightNode(this._zh.found[this._zh.currentIndex].node.id)
	}
};
getOrgChart.prototype.highlightNode = function(c) {
	...
	function b() {
		...
		a
				.move(
						[ f, g, a.initialViewBoxMatrix[2],
								a.initialViewBoxMatrix[3] ],
						null,
						function() {
							...
							j[4] = j[4]
									- ((d.w / 2) * (getOrgChart.HIGHLIGHT_SCALE_FACTOR - 1));
							j[5] = j[5]
									- ((d.h / 2) * (getOrgChart.HIGHLIGHT_SCALE_FACTOR - 1));
							get._z(i, {
								transform : h
							}, {
								transform : j
...
getOrgChart.prototype._l = function() {
	if ((this._zh.currentIndex < this._zh.found.length - 1)
			&& (this._zh.found.length != 0)) {
	...
	if ((this._zh.currentIndex != 0) && (this._zh.found.length != 0)) {
		this._X._a9.className = this._X._a9.className.replace(" get-disabled",
				"")
	} else {
		if (this._X._a9.className.indexOf(" get-disabled") == -1) {
			this._X._a9.className = this._X._a9.className + " get-disabled"
		}
	}
};
...
getOrgChart.prototype._aV = function(g, a) {
...
getOrgChart.prototype._zk = function(a, d, e, c) {
...
getOrgChart.prototype.expand = function(b, a) {
	...
	if ((b.parent == this._zd) || (b.parent == null)) {
...
getOrgChart.prototype._h = function(d, f, g, c, a, b) {
	var h = this.config.customize[d] && this.config.customize[d].theme ? getOrgChart.themes[this.config.customize[d].theme]
			: this.theme;
	a = (a == undefined ? getOrgChart.NOT_DEFINED : a);
	var e = new getOrgChart.node(d, f, g, c, h.size, this.config.photoFields,
			a, b);
	...
	if (this.nodes[d]) {
		e._zH = this.nodes[d].x;
		e._zN = this.nodes[d].y
	} else {
		e._zH = null;
		e._zN = null
	}
...
getOrgChart.prototype.loadFromJSON = function(g, p, a) {
	...
	if (p) {
		for ( var e in g) {
			...
			this._h(e, g[e].pid, g[e].spid, g[e].data, g[e].collapsed,
					g[e].color)
		}
	} else {
		...
		for ( var e in this.nodes) {
			...
			if (k.children.length == 0) {
				for (z = n.children.length - 1; z >= 0; z--) {
					var b = n.children[z];
					if (b.children.length == 0 && b != this.nodes[j]
							&& b.displayPid == null
							&& b.displayChildren == null) {
						if (this.nodes[j].color == null) {
							this.nodes[j].color = getOrgChart.util
									._1(this.config.color)
						}
...
get._a._zi = function(f, a, d, c, b, e) {
	var g = get._a._zY();
	g.open(d, f, e);
...
get._a._T = function(g, b, a, c, f) {
	...
	get._a._zi(g + "?" + e.join("&"), a, "GET", c, null, f)
};
get._a._a5 = function(g, b, a, c, f) {
	var e = [];
	for ( var d in b) {
		e.push(encodeURIComponent(d) + "=" + encodeURIComponent(b[d]))
	}
	get._a._zi(g, a, "POST", c, e.join("&"), f)
};
getOrgChart.prototype._y = function(d, b) {
	...
	if (b[0].touches.length == 2) {
		this._aK.dragStart = null;
		this._p = null;
		this._a1 = getOrgChart.util._aP(this._X._v, a)
	}
};
getOrgChart.prototype._b = function(j, d) {
	...
	if (d[0].touches.length == 1) {
		...
		if (this._aK.dragStart) {
			...
			var f = l[3] / this._aO;
			var a = m > f ? m : f;
			l[0] = -((g - this._aK.dragStart.x) * a)
					+ this._aK.dragStart.viewBoxLeft;
			l[1] = -((h - this._aK.dragStart.y) * a)
					+ this._aK.dragStart.viewBoxTop;
			l[0] = parseInt(l[0]);
			l[1] = parseInt(l[1]);
	...
	if (d[0].touches.length == 2) {
		var b = getOrgChart.util._aP(this._X._v, d[0]);
		var k = 1 + ((b - this._a1) / (this._a1 / 100)) / 100;
		var i = getOrgChart.util._a2(this._X._v, d[0]);
		this.zoom(k, null, i);
		this._a1 = b
	}
};
...
getOrgChart.prototype._aY = function(i, c) {
	var b = c[0];
	b.preventDefault();
	this._aR(i, c);
	this._aV(i, c);
